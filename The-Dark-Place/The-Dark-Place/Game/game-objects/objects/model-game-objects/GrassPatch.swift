...
class GrassPatch: InstancedGameObject {
    ...
    init(patchWidth: Int, patchHeight: Int, grassCount: Int){
        super.init(meshType: .Quad_Custom, textureType: .StandingGrass, instanceCount: grassCount)
        setName("Instanced Grass")
        
        let halfWide: Float = Float(patchWidth / 2)
        let halfBack: Float = Float(patchHeight / 2)
